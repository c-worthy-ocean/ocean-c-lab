...
def co2_eq_const(S, T):
    """Compute carbonate system equilibrium constants"""
    ...
    eq["k_hbo2"] = np.exp(
        (-8966.90 - 2890.53 * s_sqrt - 77.942 * S + 1.728 * s_1p5 - 0.0996 * s_sq) * t_inv
...
def newton_safe(funcd, x_guess, x1, x2, xacc, **kwargs):
    ...
    for j in range(0, MAXIT):
        if ((rts - xh) * df - f) * ((rts - xl) * df - f) >= 0 or abs(2.0 * f) > abs(dxold * df):
            dxold = dx
            dx = 0.5 * (xh - xl)
            rts = xl + dx
            if xl == rts:
...
def _calc_pH_from_alk_pco2(
    ...
    df = (
        ...
        - k_oh / h_2
        + (k_12p * (a - h * da) - 2.0 * k_123p * da - h_2 * (3.0 * a - h * da)) * phosphate / a2
...
def _calc_pH_from_dic_alk(
    ...
    df = (
        ...
        - k_oh / x_2
        + (k_12p * (a - x * da) - 2.0 * k_123p * da - x_2 * (3.0 * a - x * da)) * phosphate / a2
...
def calc_co2(
    ...
    co2aq = (
        dic_loc
        * h2
        / (h2 + thermodyn["k_h2co3"] * h_total + thermodyn["k_h2co3"] * thermodyn["k_hco3"])
    )
    if input_in_gravimetric_units:
        return co2aq * 1.0e6  # µmol/kg
    else:
        return co2aq * rho_ref * 1.0e3  # covert to mmol/m^3
...
def calc_dic(
    ...
    pco2_loc = pCO2 * 1e-6
    k0 = 1e-6 * co2sol(S, T, return_in_gravimetric_units=True)

    h_total = np.vectorize(newton_safe)(
        _calc_pH_from_alk_pco2,
        10.0 ** (-pH0),
        10.0 ** (-co2_ph_low),
        10.0 ** (-co2_ph_high),
        co2_chem_tol,
    ...
    h2 = h_total * h_total
    co2aq = k0 * pco2_loc  # mol kg^{-1}

    dic = co2aq * (
        1.0 + thermodyn["k_h2co3"] / h_total + thermodyn["k_h2co3"] * thermodyn["k_hco3"] / h2
    )
...
def revelle_factor(S, T, dic, co2, pH, gamma=0.0, thermodyn=None):
    """Compute the Revelle Factor
    ...
    h2 = h * h
    h3 = h * h * h
    k1 = thermodyn['k_h2co3']
    k2 = thermodyn['k_hco3']
    k1k2 = k1 * k2
    kb = thermodyn['k_hbo2']
    bt = thermodyn['boron_total']
    kb_p_h_sq = (kb + h) ** 2
    kw = thermodyn['k_oh']
...
def ddicdco2(S, T, dic, co2, pH, thermodyn=None):
    """
...
def rf_ddicdco2(S, T, dic, co2, pH, gamma=0.0, thermodyn=None):
    """Compute the Revelle Factor
    ...
    revelle_factor = c0 + gamma * c1

    return revelle_factor, ddicdco2
